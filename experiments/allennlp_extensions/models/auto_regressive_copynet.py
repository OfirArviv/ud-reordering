...
@SeqDecoder.register("auto_regressive_seq_decoder_copynet")
class AutoRegressiveSeqDecoderCopynet(SeqDecoder):
    """
    ...
    def __init__(
        ...
        # region CopyNet
        if not next(self._decoder_net.parameters()).is_cuda:
            self._copy_attention = BilinearAttention(vector_dim=self._decoder_net.get_output_dim(),
                                                     matrix_dim=self._decoder_net.target_embedding_dim,
                                                     normalize=False)
        else:
            self._copy_attention = BilinearAttentionCuda(vector_dim=self._decoder_net.get_output_dim(),
                                                         matrix_dim=self._decoder_net.target_embedding_dim,
                                                         normalize=False)

        vocab_size = self._vocab.get_vocab_size(target_namespace)
        target_vocab_size = vocab_size - pointer_vocab_size
    ...
    def _copy(self, decoder_output: torch.Tensor, encoder_output: torch.Tensor):
        ...
        # We multiply the target_embedding to match the decoder_output shape. Each 'batch' need to be multiply
    ...
    def _forward_loss(
        ...
        if self._scheduled_sampling_ratio == 0 and self._decoder_net.decodes_parallel:
            ...
            # region CopyNet
            copy_logits = self._copy(decoder_output, encoder_outputs)
            ...
            logits = torch.cat([logits, copy_logits], dim=2)
    ...
    def _prepare_output_projections(
        ...
        assert decoder_state == {}, "Current implementation of CopyNet do not support no-empty decoder step. " \
                                    "Please update the code to support it."

        copy_logits = self._copy(decoder_output, encoder_outputs)
        padding_size = self._pointer_vocab_size - copy_logits.shape[-1]
        copy_logits = torch.nn.functional.pad(input=copy_logits, pad=(0, padding_size, 0, 0),
                                              mode='constant', value=0)
        output_projections = torch.cat([output_projections, copy_logits], dim=1)
    ...
    def forward(
        ...
        if not self.training:
            ...
            if target_tokens:
                ...
                output_dict["target_tokens"] = self.indices_to_tokens(targets[:, 1:])
                if self._tensor_based_metric is not None:
                    for metric in self._tensor_based_metric:
                        metric(best_predictions, targets)  # type: ignore
                ...
                if self._token_based_metric is not None:
                    ...
                    for metric in self._token_based_metric:
                        metric(predicted_tokens, output_dict["target_tokens"])  # type: ignore

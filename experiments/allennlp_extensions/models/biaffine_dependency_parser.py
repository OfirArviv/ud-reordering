...
# from final.allennlp_extensions.metrics.dependency_f1_score import DependencyFBetaMeasure


from experiments.allennlp_extensions.metrics.dependency_attachment_score import AttachmentScores
from experiments.allennlp_extensions.predictors.dependency_conllu_predictor import DependencyConlluPredictor
...
class BiaffineDependencyParser(Model):
    ...
    def __init__(
        ...
        metric: str = "attachment",
        metric_file_path=None,
        prediction_file_path=None,
        labels_to_mask: Union[List[str], str] = None,
        directions_to_mask: Union[List[str], str] = None,
        component_to_mask: str = "all",
        use_label_mask_in_score: bool = True,
        positional_encoding: str = None,
        **kwargs,
    ) -> None:
        super().__init__(vocab, **kwargs)

        if labels_to_mask is not None:
            assert isinstance(labels_to_mask, List) or isinstance(labels_to_mask, str)
            if isinstance(labels_to_mask, List):
                assert all(isinstance(x, str) for x in labels_to_mask)
                self.labels_to_mask = list(map(lambda label: vocab.get_token_index(label, "head_tags"), labels_to_mask))
            else:
                assert labels_to_mask == "all"
                self.labels_to_mask = list(range(vocab.get_vocab_size("head_tags")))
        else:
            self.labels_to_mask = []

        if directions_to_mask is not None:
            assert isinstance(directions_to_mask, List) or isinstance(directions_to_mask, str)
            if isinstance(directions_to_mask, List):
                assert all(isinstance(x, str) for x in directions_to_mask)
                self.directions_to_mask = directions_to_mask
            else:
                self.directions_to_mask = [directions_to_mask for i in self.labels_to_mask]
        else:
            self.directions_to_mask = []

        assert len(self.labels_to_mask) == len(self.directions_to_mask)

        assert component_to_mask in ["all", "label_only", "head_only"]
        self.component_to_mask = component_to_mask
        ...
        if metric == "attachment":
            score_label_mask = self.labels_to_mask if use_label_mask_in_score else []
            score_direction_mask = self.directions_to_mask if use_label_mask_in_score else []
            self._attachment_scores = AttachmentScores(ignore_classes=score_label_mask,
                                                       ignore_directions=score_direction_mask)
        # elif metric == "f_measure":
        #     self._attachment_scores = DependencyFBetaMeasure(self.vocab.get_index_to_token_vocabulary("head_tags"),
        #                                                      metric_file_path=metric_file_path)
        elif metric == "conllu_predictor":
            self._attachment_scores = DependencyConlluPredictor(self.vocab.get_index_to_token_vocabulary("head_tags"),
                                                                output_path=prediction_file_path)
        else:
        ...
        if positional_encoding is None:
            ...
            self.positional_embedding = nn.Embedding(positional_embedding_size, representation_dim)
            logger.info("Using positional encodings!")
        else:
    ...
    def forward(
        ...
        if self.positional_encoding:
            position_ids = torch.arange(embedded_text_input.size()[1], dtype=torch.long, device=embedded_text_input.device)
            position_ids = position_ids.unsqueeze(0).expand(embedded_text_input.shape[:-1])
            embedded_text_input = embedded_text_input + self.positional_embedding(position_ids)

        mask = get_text_field_mask(words)

        predicted_heads, predicted_head_tags, mask, arc_nll, tag_nll, tag_confidence = self._parse(
        ...
        if head_indices is not None and head_tags is not None:
            if isinstance(self._attachment_scores, DependencyConlluPredictor):
                for meta, heads, tags, mask, confidence in zip(metadata, predicted_heads[:, 1:],
                                                               predicted_head_tags[:, 1:],
                                                               mask[:, 1:],
                                                               tag_confidence[:, 1:]):
                    self._attachment_scores(meta["words"], meta["pos"], meta["ids"], meta["metadata"], heads, tags,
                                            mask, confidence)
            else:
    ...
    def _parse(
        ...
        if self.training or not self.use_mst_decoding_for_validation:
            predicted_heads, predicted_head_tags, tag_confidence = self._greedy_decode(
        ...
        tag_confidence = tag_confidence if tag_confidence is not None else (torch.zeros_like(predicted_head_tags) - 1)

        return predicted_heads, predicted_head_tags, mask, arc_nll, tag_nll, tag_confidence
    ...
    def _greedy_decode(
        ...
        confidence, head_tags = torch.nn.functional.softmax(head_tag_logits, dim=2).max(dim=2)
        # _, head_tags = head_tag_logits.max(dim=2)
        return heads, head_tags, confidence

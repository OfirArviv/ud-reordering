...
def _convert_bare_closing_parentesis_to_full(decoupled_form_string: str, use_multilingual_top_format: bool):
    ...
    for s_idx in matching_parentheses_dict.keys():
        ...
        if use_multilingual_top_format:
...
def _parse_tree_to_parse_str(parse_tree: nltk.Tree, use_multilingual_top_format: bool) -> str:
    tree_str = str(parse_tree).replace("(", "[").replace(")", "]").replace("{", "(").replace("}", ")") \
        .replace("\n ", "").replace("]", " ]")
    assert "\n" not in tree_str
    assert "[[" not in tree_str

    return _convert_bare_closing_parentesis_to_full(tree_str, use_multilingual_top_format)
...
def get_parse_tree_str_from_pointer_tree(parse_tree: nltk.Tree, source_sequence: str, use_pointers: bool,
    ...
    target_sequence = _parse_tree_to_parse_str(parse_tree, use_multilingual_top_format)
...
def create_mtop_seq2seq_dataset(input_file_path: Union[str, Tuple[str, str]],
                                ...
                                reorder_by_lang: Optional[str] = None,
                                multilingual_top_format: bool = False):
    if reorder_algo:
        ud_reorder_algo = UdReorderingAlgo(reorder_algo, input_lang)
    ...
    with open(input_file_path, "r", encoding='utf-8') as input_file:
        for line_num, line in tqdm(enumerate(input_file)):
            ...
            _id, intent, slot_string, utterance, domain, locale, decoupled_form_string, token_json = line_parts
            source_sequence = " ".join(json.loads(token_json)['tokens'])
            if multilingual_top_format:
                target_sequence = multilingual_top_format_to_mtop_format(target_sequence)
                pointers_parse_tree = _get_parse_tree_from_str(target_sequence)
            else:
...
if __name__ == "__main__":
    ...
    #     "experiments/datasets/top/multilingual-top-main/processed_data/en/train.nl",
    #     "experiments/datasets/top/multilingual-top-main/processed_data/en/train.mrl",
    #     "english",
    #     True,
    #     True,
    #     "temp",
    ...
    # create_english_dataset_script(True)

    # Create Reordered Dataset
    create_reordered_datasets_scripts(True)

    # Create Test Dataset
    # create_test_datasets_script(True)

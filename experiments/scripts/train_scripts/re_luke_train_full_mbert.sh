...
export config_file="re_luke-mbert.jsonnet"
MODEL_IDX="$EXPERIMENT_NUM"

dataset_dir=experiments/processed_datasets/smiler/
# Standard Order Model
export train_data_path="$dataset_dir"/standard/en-small_corpora_train.tsv.json
...
sbatch $sbatch_params -J train_re_luke experiments/scripts/train_scripts/train_subscript_small.sh
...
do
  ...
  do
    ...
    do
      export train_data_path="$dataset_dir"/"$subdir"/en-small_corpora_train_reordered_by_"$lang"_"$algo""$combined_postfix".tsv.json

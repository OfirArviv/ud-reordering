...
dataset_dir=experiments/processed_datasets/tacred_small/standard_split
languages=(korean russian)
algo_arr=(HUJI RASOOLINI)
count_arr=( 100 200 300 )
for lang in "${languages[@]}"
do
  export train_data_path="$dataset_dir"/"$lang"_train.conllu.json
  export valid_data_path=null
  export test_data_path="$dataset_dir"/"$lang"_test.conllu.json
  ...
  do
    ...
    sbatch $sbatch_params -J ft_tacred experiments/scripts/train_scripts/finetune_scripts/finetune_subscript.sh
  done
done
